...
public class StringUtils {
	...
	//方法3：返回参数1个中文汉字字符串，字符集必须在GB2312(相当于中文简体)范围内，例如“中呀被”(5分)
	...
	public static String randomChineseString(int length){
		...
		for (int i = 0; i < length; i++) {
			buffer.append(randomChineseString());
		}
	...
	//方法4：返回中文姓名，必须以真实姓开头，百家姓在“六、附百家姓”里，名使用1-2个随机汉字(8分)，内部调用randomChineseString()方法(3分)。返回示例：“刘呀被”、“欧阳及为”
		public static String getName(){
			...
			String first = str[RondomUtils.random(0, str.length-1)];
			String last = randomChineseString(RondomUtils.random(1, 2));
			return first + last;
		}
